...
| 項目 | 現況 |
|---|---|
| 最新發行版 | [VisionFlow AOI v1.10.1](https://github.com/wjcudalearning/VisionFlow/releases/tag/v1.10.1)，Windows x64、CUDA `sm_86` |
...
- Sensor 中繼（研華 PCIe-1730）：現場 Sensor 接 I/O 卡 DI、I/O 卡 DO 接擷取卡，必須有程式轉送。CCD 頁「Sensor 中繼」面板可設定 DI／DO 通道、有效電位、DO 脈寬、防抖間隔與讀取間隔；啟用後外部觸發單張由程式把 DI 變有效轉成 DO 脈衝，軟體觸發改由 Sensor 起拍。預設關閉，原機台程式也控制這張卡，兩者不可同時開啟。需要機台安裝 DAQNavi（`Automation.BDaq4.dll`，不打包），缺少時只停用此功能；找不到時可在面板按「瀏覽」選取 DLL（也接受它所在的資料夾與舊版 `Automation.BDaq.dll`，貼上路徑的引號會自動去除），畫面會顯示實際檢查的路徑與原因。轉送為軟體輪詢，延遲約為讀取間隔加系統排程；面板顯示最長讀取間隔。另有「讀取 DI」「送出 DO 測試脈衝」兩個分段測試，外部觸發診斷會依 DI 次數與 DO 脈衝數指出斷在哪一段。
- 光源（RS-232）：CCD 頁「光源」面板以 .NET `SerialPort`（與原 C# 程式相同，不另裝套件）控制光源控制器，品牌不限——依原程式送的指令設定開燈／關燈指令，並以「亮度指令範本」（`{channel}`、`{value}`、`{checksum}`／`{xor}` 校驗碼，例如 `@{channel:02}F{value:03}{checksum}`）逐通道調整光強。啟用後相機直連監控開始時自動開燈並送出各通道亮度、停止時關燈；其他時候用「開燈／關燈」手動測試，關閉程式時若燈仍開著會關燈。所有串列埠通訊在單一背景執行緒上依序執行；COM port 同時只能一個程式使用，原機台程式須先關閉。
- 從原機台程式匯入（智能模式，管理模式）：CCD 頁選擇原機台 C# 程式的 `.sln`／`.csproj`，只讀取原始碼與設定檔（不執行），穿過包裝類別、方法參數與具名／預設引數、建構子、欄位、常數、列舉、`switch`、方法回傳值、`DllImport` 別名與設定檔（App.config、.settings、INI，含 `bin` 下執行時存的設定），找出米輪（自動遞增、倍頻、CMP Out Width、反向、卡片 ID）、Sensor 中繼（裝置、DI／DO、脈寬、有效電位，含表單 `.resx` 裡的裝置名稱）、光源（COM port 參數、開燈／關燈指令，並把 `$"..."`／`string.Format` 組成的亮度指令轉成範本）與 CCF／影像長度／曝光／增益，列出確認表與出處。只有追到唯一固定值的「可套用」會預設勾選；會被畫面輸入或讀檔改寫的「可能只是預設值」、多處不同與追不到的值不會自動套用。套用走 CCD 頁既有的設定路徑（產品參數成為 Recipe 未儲存修改），不會自動啟用 Sensor 中繼。
- 相機參數分兩層：機台層（Sapera server／resource、CCF、米輪卡片與 CMP0–7、Sensor 中繼）存 `config/ccd_machine.json`；產品層（曝光、增益、影像長度、內部線速率、觸發、自動存圖）存 Recipe 選用 `camera` 區段。
