# Detector 202-CS-SN-1 自動 CNR 演算法評估
...
Detector 202-CS-SN-1 的主要優勢不是「完全不用門檻」，而是把固定灰階門檻改成依影像雜訊自動調整的局部異常門檻。它先估計緩慢變化的背景，再對背景扣除後的 residual 使用 MAD（Median Absolute Deviation，中位絕對偏差）估算雜訊，因此對整張影像一起變亮或變暗，比固定二值化 `gray > 172` 穩定很多。

不過，它仍有一個固定的絕對門檻下限 `8 DN`，而且 `3 × 3 Open` 會消除過細或過弱的訊號。所以「Auto CNR 可以承受任意光衰」並不成立；當缺陷 residual 衰減到約 8 個灰階以下，或雜訊相對缺陷變得太大時，仍會失效。

本次使用當時的 Detector 202 與目前的 202-CS-SN-1 程式做合成影像壓測。在「背景 150 DN、亮缺陷 220 DN、11 × 11 px」的案例中：
...
因此，目前對 Detector 202-CS-SN-1 最合理的工程結論是：

> 合成模型支持約 `40%` 的光衰容忍能力；在尚未使用實際相機、鏡頭、材料與真實缺陷驗證前，建議只把 `30%` 當成暫定設計容忍值，不應直接把 `40%` 或 `71%` 寫成量產保證規格。

這個 `30%` 是保留模型誤差與實機差異後的工程建議，不是演算法的硬性數學常數。依目前補充的實際背景灰階，整張圖中間約 `170 DN`，左右兩側切圖各自平均約 `100 DN`，而左右切圖各分成五等分後，已知最低約 `80 DN`、最高約 `140 DN`。這顯示影像本身存在顯著的空間亮度不均；因此 `30%` 必須解讀成「每一分區相對於自身良品基準的後續光衰」，不能直接以中間 `170 DN` 當成左右區域的共同基準。最終數字仍必須以實際資料做分區光衰驗證。
...
目前提供的是背景亮度範圍，尚未包含實際缺陷灰階、black level、各分區雜訊或五個分區的完整數值：

| 區域 | 背景灰階約值 | 相對中間亮度 | 相對中間降低 |
|---|---:|---:|---:|
| 中間 | `170 DN` | `100.0%` | `0%` |
| 左右兩側各自平均 | `100 DN` | `58.8%` | `41.2%` |
| 左右五等分中的最低值 | `80 DN` | `47.1%` | `52.9%` |
| 左右五等分中的最高值 | `140 DN` | `82.4%` | `17.6%` |

計算方式為：

$$
r_z=\frac{\mu_z}{\mu_{center}}
$$
...
$$
D_{side}=1-\frac{100}{170}=41.18\%
$$

最暗分區相對中間的亮度差為：

$$
D_{min}=1-\frac{80}{170}=52.94\%
$$

左右五等分目前只知道最低 `80 DN` 與最高 `140 DN`，不能據此推定其餘三區的數值或分布。已知範圍跨度為 `60 DN`；若只作亮度不均的描述，Michelson contrast 為：

$$
C_M=\frac{140-80}{140+80}=27.27\%
$$

此數字描述左右區域內的空間不均，**不是缺陷 CNR，也不是 LED 已衰退 `27.27%`**。

### 空間亮度不均不等於時間光衰

中間 `170 DN`、左右平均 `100 DN` 的差異，是同一時間點不同位置的基準亮度差，可能來自照明角度、暗角、材料反射或切圖位置。它和同一區域隨使用時間從基準值下降的「時間光衰」是兩個不同維度。

因此不能直接說左右區域已經消耗 `41.2%` 光衰容忍額度，也不能以：

$$
1-\frac{100}{170}
$$

代表光源老化程度。正確作法是每個固定分區 $z$ 都用自己的初始基準，並扣除 black level：

$$
a_z(t)=\frac{\mu_z(t)-I_{black}}{\mu_z(0)-I_{black}}
$$
...
例如忽略 black level、只作直觀換算時，若各區相對自身基準再衰退 `30%`：

| 分區基準 | `30%` 後約值 | `40%` 後約值 |
|---:|---:|---:|
| 中間 `170 DN` | `119 DN` | `102 DN` |
| 左右平均 `100 DN` | `70 DN` | `60 DN` |
| 左右最低 `80 DN` | `56 DN` | `48 DN` |
| 左右最高 `140 DN` | `98 DN` | `84 DN` |

這張表不是通過／失敗門檻；它指出最暗帶在後續光衰後會進入更低 DN 的工作區，此時 read noise、black level、量化誤差與相機非線性占比會升高。先前以背景 `150 DN` 建立的合成模型，對 `80 DN` 最暗帶可能偏樂觀。

### 對固定門檻 172 的直接影響

固定門檻使用 `gray > 172`：

- 中間背景約 `170 DN`，距門檻只有 `2 DN`。背景紋理或雜訊只要略微向上波動，就可能越過門檻並形成不穩定輪廓；後續面積與四邊形條件可能排除一部分，但無法消除門檻位置本身過近的風險。
- 左右背景 `80～140 DN` 雖不容易因背景直接越過 `172`，但同一類亮缺陷在暗區的絕對灰階也可能低於 `172`，造成漏檢。
- 因此單一全域門檻同時面對「中心背景太靠近門檻」與「暗區缺陷可能到不了門檻」兩個方向相反的問題。

目前沒有實際缺陷灰階，不能直接計算 Detector 202 的真實 recall。為了說明風險，可暫用原合成案例的缺陷／背景比例：

$$
q=\frac{220}{150}=1.4667
$$

若假設實際缺陷會和局部背景維持相同比例，則估計缺陷灰階為：

| 局部背景 | 假設缺陷灰階 $q\mu_b$ | 是否超過 `172` |
|---:|---:|---|
| 中間 `170 DN` | 約 `249 DN`，實際 8-bit 影像會受飽和限制 | 是 |
| 左右平均 `100 DN` | 約 `147 DN` | 否 |
| 左右最低 `80 DN` | 約 `117 DN` | 否 |
| 左右最高 `140 DN` | 約 `205 DN` | 是 |

在這個**假設模型**下，缺陷恰好跨過門檻時的局部背景為：

$$
\mu_{b,critical}=\frac{172}{1.4667}\approx117.3\ DN
$$

所以低於約 `117 DN` 的暗區容易漏掉同相對對比的缺陷。這只是一個敏感度示例，不是實測結果；如果真實缺陷是固定增加若干 DN、會飽和、或反而比背景暗，結果都會不同。

### 對 Auto CNR 的影響

Auto CNR 先以局部 Gaussian 背景扣除緩慢亮度變化，所以左右平均比中間暗這件事，不會像固定 `172` 一樣直接決定候選是否存在。只要五等分之間的亮度變化相對平滑，且空間尺度大於 Gaussian 背景 kernel，這類暗角或漸層多半會被背景模型吸收。

但 Auto CNR 並非完全不受局部亮度影響：

- 若五等分邊界有突然的亮度跳變，且尺度接近背景 kernel `31～151 px`，邊界可能留在 residual 中，形成誤報候選。
- 最暗帶只有約 `80 DN`；光衰後 shot-noise CNR、read noise 占比、black level 與 `8 DN` residual 下限都可能更早成為限制。
- Detector 目前是先以 residual 找候選，再計算 CNR；局部 CNR 高低尚未直接 gate NG，因此暗區是否穩定仍要同時看候選數與誤報率。

若先忽略 read noise，並假設缺陷訊號差與局部光量成正比、shot noise 標準差與光量平方根成正比，則 CNR 大致隨 $\sqrt a$ 變化：

$$
\frac{CNR_z}{CNR_{center}}\approx\sqrt{\frac{\mu_z}{170}}
$$

| 區域 | shot-noise 模型下相對中間 CNR | CNR 約降低 |
|---|---:|---:|
| 左右平均 `100 DN` | `76.7%` | `23.3%` |
| 左右最低 `80 DN` | `68.6%` | `31.4%` |
| 左右最高 `140 DN` | `90.7%` | `9.3%` |

這表示「局部背景可自動適應」不等於「不同亮度區域擁有完全相同的檢出能力」。加入不隨光量下降的 read noise 後，最暗帶通常會比上表更不利。

### 更新後的工程判斷

加入實際背景數值後，評估結論更新如下：

1. 這組數據更強烈支持使用局部背景／residual 方法，而不是一個全圖固定 `172`；固定門檻難以同時照顧中間與兩側。
2. 現有數據只描述背景，不足以驗證真實缺陷 recall，也不足以把 `30%` 升級成量產保證。
3. 原本建議的 `30%` 暫定光衰值不提高；它應對每個分區分別由自身基準計算。
4. 左右最低約 `80 DN` 的分區是優先驗證區。因先前合成背景為 `150 DN`，其 `40%` 保守模型結果不能直接外推到最暗帶。
5. 正式驗證至少要保存十個側邊分區（左五區、右五區）各自的完整基準灰階、black level、局部雜訊、正常品候選數，以及各缺陷類別的 $\mu_d$、$\mu_b$、$\sigma_b$、CNR 與 recall。

## 重要觀念：它是「先找異常，再計算 CNR」

雖然名稱是 Auto CNR，但目前 Detector 202-CS-SN-1 並不是直接使用 `CNR > 某門檻` 來決定候選。
...
Detector 202-CS-SN-1 的 Gray 使用專案共用 `PreprocessPlan`；CPU 以 OpenCV 結果為正確性基準。後續 Auto CNR 分析目前在 CPU 執行。
...
Detector 202-CS-SN-1 沿用原 Detector 202 的屏蔽語意：
...
| Detector 202-CS-SN-1 | AcceptanceChecker Auto CNR 邏輯 |
| 屏蔽 | 關閉，以隔離演算法光衰能力 |
...
| 面向 | 固定門檻 Detector 202 | Auto CNR Detector 202-CS-SN-1 |
|---|---|---|
...
Detector 202-CS-SN-1 在光源衰退時仍可維持檢出，這對缺陷檢測是優點，但不能拿它證明照明系統健康。應另外監控背景灰階、曝光、Gain、均勻性和光源輸出。
...
不要只用 LED 控制器的設定百分比。應以扣除 black level 後的有效背景灰階量測，而且中間、左五區、右五區應分開計算：
...
其中 $\widetilde I_{bg}$ 建議使用固定背景 ROI 的 median，避免少數缺陷或反光影響。對分區 $z$，應保存 $a_z$ 與 $L_z$，最後以最差分區作為照明健康判定；不能把中間 `170 DN` 當成左右 `80～140 DN` 的 baseline。相機 exposure、Gain、gamma、white balance 與 ISP 必須固定，否則量到的是相機補償後亮度，不是真正的光衰。
...
2. 中間、左五區、右五區各自用初始灰階作 baseline，分別定義 `0%、10%、20%、30%、40%、50%` 光衰；不可用中間 `170 DN` 統一換算兩側。
3. 每個光衰階段至少包含：
   - 正常品。
   - 亮極性 NG。
   - 暗極性 NG。
   - 最小重要缺陷。
   - 不同 tile 位置，至少完整覆蓋左五區、右五區、中間與靠近屏蔽邊界的位置。
4. 每一類至少收集多張獨立影像，不要只把同一張圖做數位乘法。
5. 同時執行保留的內部 Detector 202 參考實作與 202-CS-SN-1，比較：
   - 目標定位 recall。
   - 每張圖誤報 component 數。
   - `robust_noise_sigma`。
   - `residual_threshold`。
...
- **演算法暫定設計容忍值：`30%` 光衰。**
- **合成模型保守能力估計：背景 `150 DN` 的指定模型約 `40%` 光衰；不可直接外推到背景約 `80 DN` 的最暗帶。**
...
1. 已知中間背景約 `170 DN`、左右平均約 `100 DN`、側邊五等分範圍約 `80～140 DN`；尚缺左五區與右五區的完整個別數值、重要缺陷灰階、black level 與各區雜訊。
2. 已知基準影像存在空間亮度不均；尚待確認隨時間的光衰是各區同比下降，還是暗角、色偏與局部不均會進一步擴大。
...
- 光衰試驗直接呼叫工作區的 Detector 202 參考實作與 202-CS-SN-1；合成影像在記憶體產生，未作為 production fixture 提交。
