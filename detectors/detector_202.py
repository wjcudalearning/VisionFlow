...
class Detector202(BaseDetector):
    """Unregistered legacy detector kept as the mask base for ``Detector202_1``.

    Only the exclusion geometry helpers are used by the registered production
    Detector202_1. This class's detector, threshold, contour, and legacy parameter
    paths are not registered or used by production recipes.
    """
    ...
    PARAM_SPEC = {
        **specs_from_defaults(
            default_params,
            {
                ...
                **edge_mask_parameter_overrides(enabled_label="啟用邊緣屏蔽"),
                "threshold_value": {
                    "minimum": 0,
                    "maximum": 255,
                    "label": "一般二值化門檻",
    ...
    def detect(self, image) -> list[dict]:
        ...
        effective_edge_insets = self._effective_edge_insets(image_width, image_height)
        base_meta = {
            "shape": "quadrilateral",
            "vertices_local": None,
            "vertex_count": 4,
            ...
            "min_area": min_area,
            "max_area": max_area,
            "center_mask_enabled": bool(self.params.get("center_mask_enabled", True)),
            "center_mask_size": [2 * half_x, 2 * half_y],
            "center_mask_half_extents": [half_x, half_y],
            "edge_mask_enabled": edge_mask_enabled,
            "edge_insets": edge_insets,
            "effective_edge_insets": effective_edge_insets,
        ...
        for contour in contours:
            ...
            metadata.update(
                ...
                center_mask_size=list(base_meta["center_mask_size"]),
                center_mask_half_extents=list(base_meta["center_mask_half_extents"]),
                edge_insets=dict(edge_insets),
                effective_edge_insets=dict(effective_edge_insets),
            )
            defects.append(
                {
    ...
    def _exclusion_geometry(self, width: int, height: int) -> dict:
        ...
        if bool(self.params.get("center_mask_enabled", True)):
            ...
            x_start, y_start, mask_width, mask_height = center_mask_bbox(
                width, height, center_x, center_y, half_x, half_y
            )
            if mask_width > 0 and mask_height > 0:
                center = (x_start, y_start, x_start + mask_width, y_start + mask_height)
    ...
    def _apply_exclusion_masks(self, binary: np.ndarray) -> np.ndarray:
        ...
        return apply_edge_insets(masked, insets, copy=False)
    ...
    def _passes_area_filter(self, area: float) -> bool:
        min_area = float(self.params.get("min_area", 5.0))
        max_area = float(self.params.get("max_area", 100.0))
        return passes_area_filter(area, min_area, max_area)
