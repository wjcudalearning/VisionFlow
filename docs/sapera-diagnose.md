...
外部觸發模式開始預覽或擷取後，CCD 畫面右側的「外部觸發診斷」面板會即時判讀，停止後自動隱藏。
面板依據的資料都在畫面上：米輪這一階段走了幾格、擷取卡回報的 Sensor 觸發與「被忽略」次數、
已完成幾張、連線時從 CCF 讀出的 Sensor 輸入（來源／觸發方式／電壓，即讀回值的 `FTS`／`FTD`／`FTL`）。

| 狀態 | 意思 | 面板列出的方向 |
| --- | --- | --- |
| 等待中 | 還沒走到 2 張的長度，繼續推米輪 | — |
| Sensor 訊號沒有進到擷取卡 | 米輪走了 2 張長度，擷取卡沒有任何觸發事件 | 依可能性排序：輸入腳位、電壓設定（對照讀出的 CCF 電壓）、Sensor 本身／接線、觸發方向 |
| 收到但被忽略 | 擷取卡有收到 Sensor 訊號（接線與電壓是通的），但在忙 | 上一張沒收滿行觸發（查 CMP_OUT）、取像前就被遮擋 |
| 影像收不到每一行 | 已觸發，但米輪走過 1.5 倍長度影像仍未完成 | CMP_OUT 到行觸發的接線、自動遞增設定 |
| 觸發時序異常 | 擷取卡回報行觸發太快／太慢或 Sensor 時序錯誤 | 自動遞增、倍頻、輸送速度 |

每個問題只跳一次提示，詳細內容留在面板上（文字可選取）；同樣內容也寫進 log。
面板最後的「分辨測試」是暫時取消「外部觸發單張」重新連線：有出圖代表米輪行觸發正常、問題只在 Sensor 線路。
觸發方向設反通常只會讓觸發時間點不對，很少造成完全收不到，因此排在最後。
