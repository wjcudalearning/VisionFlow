...
- [x] Detector 503-CS-AP-1：固定執行 Gray → 一般二值化（預設門檻 200、不反相）→ 中心與四邊屏蔽 → LIST contours → 多邊形近似；中心半寬／半高、四邊內縮與面積上下限為工程外參，其餘屏蔽、影像與演算法設定為管理內參，抓到任一符合候選即 NG。
