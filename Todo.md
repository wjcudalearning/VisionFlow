...
## 完成紀錄
- [x] 2026-08-19：準備 VisionFlow AOI v1.4.0 CPU-compatible Windows x64 發行；GUI Pipeline 版本同步為 1.4.0，發行內容涵蓋 Detector 202 最終語意修正、新增 `202-CS-SN-1`／`203-AS-SN-1`／`505-AS-SN-1`、正式 Detector ID 與舊 Recipe alias 相容、工程／管理內外參權限分層，以及大量 Overlay／Results 顯示穩定性改善。現有 `gpu/visionflow_cuda.dll` 並非針對本次 release commit 重編及 RTX 3090 驗證，因此發行包明確排除該 DLL；CPU-only 完整受支援，`gpu.mode=auto` 在缺 DLL 時安全回退 CPU，未宣稱本版 CUDA runtime 已驗證。
